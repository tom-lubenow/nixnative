...
rec {
  ...
  mkToolchain =
    ...
    in
    {
      ...
      supportsLTO =
        compiler.hasCapability "lto" && linker.hasCapability "lto";
      ...
      supportsThinLTO =
        ...
        in
        compilerLTO != null &&
        (compilerLTO.thin or false) &&
        linkerThinLTO;

      # Wrap library flags for linking (handles --start-group on Linux)
      wrapLibraryFlags = libs:
        linker.wrapLinkFlags { platform = targetPlatform; flags = libs; };

      # Get platform-specific linker flags
      getPlatformLinkerFlags =
        linker.platformFlags targetPlatform;

      # Get all default C flags
      getDefaultCFlags =
        compiler.defaultCFlags or [];

      # Get all default C++ flags
      getDefaultCxxFlags =
        compiler.defaultCxxFlags or [];
  ...
  makeToolchainName = compiler: linker:
    "${compiler.name}-${linker.name}";

  # Validate that compiler and linker are compatible
  validateToolchain = toolchain:
    let
      required = [ "name" "compiler" "linker" "ar" "targetPlatform" ];
      missing = builtins.filter (f: !(toolchain ? ${f})) required;
    in
    if missing != []
    then throw "nixnative: toolchain missing required fields: ${lib.concatStringsSep ", " missing}"
    else if !(toolchain.compiler ? cc)
    then throw "nixnative: toolchain compiler is missing 'cc' field"
    else if !(toolchain.linker ? driverFlag)
    then throw "nixnative: toolchain linker is missing 'driverFlag' field"
    else toolchain;
  ...
  getCapabilities = toolchain:
    ...
    in {
      ...
      lto =
        if compilerCaps.lto or null == null then null
        else if !(linkerCaps.lto or false) then null
        else compilerCaps.lto;

      thinLto =
        (compilerCaps.lto.thin or false) && (linkerCaps.thinLto or false);
      ...
      splitDwarf =
        (compilerCaps.splitDwarf or false) && (linkerCaps.splitDwarf or false);
  ...
  toolchainSupports = toolchain: feature:
    let caps = getCapabilities toolchain;
    in
    if feature == "lto" then caps.lto != null
    else if feature == "thinLto" then caps.thinLto
    else if feature == "sanitizers" then caps.sanitizers != []
    else caps.${feature} or false;
}
